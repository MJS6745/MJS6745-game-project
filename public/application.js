...
var clearBoard = function clearBoard(array) {
...
var userSelectO = function userSelectO() {
  gameStartFlag = true;
...
var checkForWin = function checkForWin(array) {
  var playerSymbol = '';
  var winFlag = false;
  ...
  if (!gameArray.some(function (item) {
    return item === '';
  })) {
...
var checkForInput = function checkForInput(id) {
  var gameSquare = 0;
  if (id === 'zero') {
    gameSquare = 0;
  } else if (id === 'one') {
...
var makeSelection = function makeSelection(event) {
  ...
  if (gameArray.some(function (item) {
    return item === '';
  }) && checkForInput(event.target.getAttribute('id')) && gameStartFlag) {
    var gameCell = event.target.getAttribute('id');
    var currentIndex = 0;
    var currentValue = '';
    if (playerTurn === 1) {
  ...
  } else if (gameArray.some(function (item) {
    return item === '';
  }) && !checkForInput(event.target.getAttribute('id')) && gameStartFlag) {
    return false;
  } else {
...
var apiUpdateGame = function apiUpdateGame(data) {
...
var apiGetGames = function apiGetGames() {
...
module.exports = {
  newGame: newGame,
  apiUpdateGame: apiUpdateGame,
  apiGetGames: apiGetGames
};
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(0)))
...
var createGame = function createGame() {
  ...
  gameApi.newGame().then(gameUi.createGameSuccess).catch(gameUi.createGameFailure);
};
...
var updateGame = function updateGame(data) {
  ...
  gameApi.apiUpdateGame(data).then(gameUi.updateGameSuccess).catch(gameUi.updateGameSuccess);
};
var getGames = function getGames(event) {
  event.preventDefault();
  // console.log('Get games in gameapi invoked')
  if (store.user !== undefined && store.user !== null) {
    gameApi.apiGetGames().then(gameUi.getGameSuccess).catch(gameUi.getGameFailure);
  } else {
...
var apiEvents = __webpack_require__(14);
...
var totalWins = function totalWins(data) {
  var totalWins = 0;
...
var totalLosses = function totalLosses(data) {
  var totalLosses = 0;
  for (var i = 0; i < data.games.length; i++) {
    if (data.games[i].over === true) {
...
var gameStats = function gameStats(data) {
  ...
  var playerLosses = totalLosses(data);
  var playerWins = totalWins(data);
...
var onSignIn = function onSignIn(event) {
  ...
  if (events.gameArray.every(function (cell) {
    return cell === '';
  })) {
...
var checkForLogin = function checkForLogin(event) {
...
var attemptLogout = function attemptLogout(event) {
  // console.log('Current store data is', store.data)
  if (store.user !== undefined) {
    return true;
  } else {
